...
router.get("/:location/:id", (req, res) => {
    ...
    if (location === "lost" || location === "found") {
        ...
        .then((snapshot) => {
            if (snapshot.exists) {
                res.render('admin/admin-retrieveItem', { data: snapshot.data(), id: req.params.id, location: location });
            } else {
...
router.post('/:location/retrieve/:id', upload.single('file'), async (req, res) => {
    try {
        ...
        if (location === "lost" || location === "found") {
            ...
            const { studentNum, firstName, email, contactNum, datetime, objTitle, sameAddress, category, description, department, retrievedBy, prevImage } = req.body;
